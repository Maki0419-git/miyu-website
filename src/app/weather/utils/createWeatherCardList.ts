		...
		time.forEach((time, index) => {
			...
			switch (elementName) {
				case "Wx":
					...
					break;
				case "PoP":
					acc[index].weatherElement.PoP = Number(time.parameter.parameterName);
					break;
				case "MinT":
					acc[index].weatherElement.MinT = Number(time.parameter.parameterName);
					break;
				case "MaxT":
					acc[index].weatherElement.MaxT = Number(time.parameter.parameterName);
					break;
			}
		});
